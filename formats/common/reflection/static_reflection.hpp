...
/*
 * copy from
 * https://bot-man-jl.github.io/articles/2018/Cpp-Struct-Field-Reflection/static_reflection.h
 */
...
#define DEFINE_STRUCT_SCHEMA(Struct, ...)               \
  template <>                                           \
  inline constexpr auto formats::StructSchema<Struct>() \
  {                                                     \
...
inline void ForEachField(T&& value, Fn&& fn)
{
  constexpr auto struct_schema = formats::StructSchema<std::decay_t<T>>();
  ...
  formats::detail::ForEachTuple(struct_schema, [&value, &fn](auto&& field_schema) {
    using FieldSchema = std::decay_t<decltype(field_schema)>;

    static_assert(std::tuple_size<FieldSchema>::value >= 2 &&
                      formats::detail::is_field_pointer_v<std::tuple_element_t<0, FieldSchema>>,
